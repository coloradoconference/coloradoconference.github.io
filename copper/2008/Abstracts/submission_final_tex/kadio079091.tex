...
\begin{center}
\rule{6in}{1pt} \
{\large Samet Kadioglu \\
{\bf Implicit Interface Tracking: Marker-Redistancing Jacobian-Free Newton-Krylov (MR-JFNK) Method}}
...
combines the {\em Lagrangian marker tracking} with the {\em Eulerian
...
the pseudo-time-steady state), and the $5^{th}$-order-accurate in space,
...
employ implicit Runge-Kutta schemes, which are $L$-stable. Since
operator-splitting errors are completely avoided, we can demonstrate
high-order-accurate (shown up to the $5^{th}$-order) solutions of the
...
to effectively cluster eigenvalues (“{\em Eigenscopy}”) of the Jacobian
matrices and the number of Krylov steps needed to converge GMRES. As the
demonstration test-cases, we use {\em solid-body translation/rotation
}(“Zalesak disk”, “Crusiform” problems), {\em moderate and severe
