...
Let $\beta_{n-1,m}$ be the constant for exact values of $\mathbf{x}_L$, then we assume
that
\begin{equation}
\tilde\beta_{n-1,m} \doteq \beta_{n-1,m} +
\frac{\partial\beta_{n-1,m}}{\partial\mathbf{x}_L}(\hat{\mathbf{x}}_L^{n-1,m}
- \mathbf{x}_L(t_{n-1,m})).
\end{equation}
...
the active part consists of 3 inverters and is weakly coupled with the slow part.
...
The BDF Compound-Fast multirate method appears to be a very powerful
method for DAEs and in particular for circuit simulation.
It is rather simple to control its local error including the interpolation errors for
coupled systems.
